...
class BinaryGate extends LogicGate {
  ...
  getPinA() {
    if (this.pinA == null) {
      return parseInt(prompt(`Enter Pin A input for ${this.getLabel()}-->`));
    } else {
  ...
  getPinB() {
    if (this.pinB == null) {
      return parseInt(prompt(`Enter Pin B input for ${this.getLabel()}-->`));
    } else {
...
class UnaryGate extends LogicGate {
  ...
  getPin() {
    if (this.pin == null) {
      return parseInt(
        prompt(`Enter Pin input for gate ${this.getLabel()} -->`)
      );
    } else {
